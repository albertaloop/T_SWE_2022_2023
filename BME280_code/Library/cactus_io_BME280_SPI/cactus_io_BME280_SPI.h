...
// structure to hold the calibration data that is programmed into the sensor in
// the factory during manufacture
...
public:
  ...
  BME280_SPI(int8_t cspin, int8_t mosipin, int8_t misopin,
             int8_t sckpin); // use this for software SPI

  bool begin(void);

  void setTempCal(float); // we can set a calibration ofsset for the
                          // temperature. this offset is in degrees celsius
