...
void FlexCAN::begin(const CAN_filter_t &mask) {
  ...
  if (mask.ext) {
    FLEXCANb_RXFGMASK(flexcanBase) = ((mask.rtr ? 1 : 0) << 31) |
                                     ((mask.ext ? 1 : 0) << 30) |
...
void FlexCAN::setFilter(const CAN_filter_t &filter, uint8_t n) {
  if (8 > n) {
    if (filter.ext) {
      FLEXCANb_IDFLT_TAB(flexcanBase, n) =
          ((filter.rtr ? 1 : 0) << 31) | ((filter.ext ? 1 : 0) << 30) |
...
int FlexCAN::write(const CAN_message_t &msg) {
  ...
  // Serial.println((uint32_t)(FLEXCANb_MBn_WORD0(flexcanBase, buffer) | 0xff)
  // >> 24); Serial.println(msg.buf[0]);
