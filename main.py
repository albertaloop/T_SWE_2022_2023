...
class Logic(Ui_MainWindow):
    ...
    def getTelemetryData(self):
        ...
        while True:
            ...
            self.battery_2_temp.setText(
                str(telemetry_manager.get_battery_temperature())
            )
            # Display battery voltage
            self.bat_1_volt.setText(str(telemetry_manager.get_battery_voltage()))
            self.bat_2_volt.setText(str(telemetry_manager.get_battery_voltage()))
            # Display acceleration
            self.accel_ind.setText(str(telemetry_manager.get_acceleration()))
            # Display battery current
            self.current_state_ind.setText(str(telemetry_manager.get_battery_current()))
            self.current_state_ind_2.setText(
                str(telemetry_manager.get_battery_current())
            )
            # Display status
            self.statusbar.setText(str(telemetry_manager.get_status()))
            # Display stripe count
            self.thing_2_label.setText(str(telemetry_manager.get_stripe_count()))
